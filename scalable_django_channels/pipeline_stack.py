...
from .deployment_stage import MyDjangoChatAppPipelineStage


class MyDjangoChatAppPipelineStack(Stack):
    def __init__(
            self,
            scope: Construct,
            construct_id: str,
        ...
        pipeline = pipelines.CodePipeline(
            ...
            docker_credentials=[
                pipelines.DockerCredential.docker_hub(
                    secretsmanager.Secret.from_secret_name_v2(
                        self,
                        "DockerHubSecret",
                        secret_name="/MyDjangoChatAppPipeline/DockerHubSecret"
                    )
                ),
            ],
            synth=pipelines.ShellStep(
                ...
                commands=[
                    ...
                    "npx cdk synth MyDjangoChatPipeline",
                ]
            ),
        )
        # Deploy to a staging environment
        self.staging_env = MyDjangoChatAppPipelineStage(
            self, "MyDjangoChatAppStaging",
            env=aws_env,  # AWS Account and Region
            django_settings_module="config.settings.stage",
            django_debug=True,
            domain_name="scalabledjango.com",
            subdomain="stage.chat",
        ...
        self.production_env = MyDjangoChatAppPipelineStage(
